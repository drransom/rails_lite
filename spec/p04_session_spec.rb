...
describe Phase4::Session do
  ...
  let(:cook) { WEBrick::Cookie.new('_rails_lite', { xyz: 'abc' }.to_json) }
  ...
  describe "#store_session" do
    context "without cookies in request" do
      ...
      it "adds new cookie with '_rails_lite' name to response" do
        cookie = res.cookies.find { |c| c.name == '_rails_lite' }
    ...
    context "with cookies in request" do
      before(:each) do
        cook = WEBrick::Cookie.new('_rails_lite', { pho: "soup" }.to_json)
        req.cookies << cook
      end
